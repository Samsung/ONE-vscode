...
(function () {
  ...
  const viewerContainer = /** @type {HTMLElement} */ document.querySelector(
    ".mondrian-viewer-bounds"
  );
  const statusLineContainer = /** @type {HTMLElement} */ document.querySelector(
    ".mondrian-statusline"
  );
  const memorySizeContainer = /** @type {HTMLElement} */ document.querySelector(
    ".mondrian-info-memory-size"
  );
  const cycleCountContainer = /** @type {HTMLElement} */ document.querySelector(
    ".mondrian-info-cycle-count"
  );
  const segmentSelect = /** @type {HTMLElement} */ document.querySelector(
    ".mondrian-segment-picker"
  );
  const viewerHScale = /** @type {HTMLElement} */ document.querySelector(
    ".mondrian-viewer-h-scale"
  );
  const viewerVScale = /** @type {HTMLElement} */ document.querySelector(
    ".mondrian-viewer-v-scale"
  );
  const scrollbarBtn = /** @type {HTMLElement} */ document.querySelector(
    ".mondrian-scrollbar-btn"
  );
  const sidePanel = /** @type {HTMLElement} */ document.querySelector(
    ".mondrian-sidepanel"
  );
  ...
  class ViewportLimiter {
    ...
    showLimit(viewer) {
      ...
      let rightRatio =
        (viewport.cycles - viewer.viewportMaxCycle) / viewport.cycles;
    ...
    updateLimit(commit = false) {
      ...
      let minCycle = Math.round(
        ((handleBound.left - barBound.left) / barBound.width) * viewport.cycles
      );
      let maxCycle = Math.round(
        ((barBound.width - barBound.right + handleBound.right) /
          barBound.width) *
          viewport.cycles
      );
    ...
    move(ev) {
      if (this.dragLeft) {
        let ratioLeft =
          (this.barOffsetLeft + ev.clientX - this.dragOrigin) /
          this.barBound.width;
      ...
      if (this.dragRight) {
        let ratioRight =
          (this.barOffsetRight - ev.clientX + this.dragOrigin) /
          this.barBound.width;
  ...
  function scaleViewport(viewer) {
    ...
    if (scaleH < 4 || scaleV < 4) {
      ...
      viewerContainer.style.backgroundImage =
        ...
        `d='M 0,${scaleV - 0.5} ${scaleH - 0.5},${scaleV - 0.5} ${
          scaleH - 0.5
        },0' ` +
        `/%3e%3c/svg%3e")`;
    }
  ...
  function updateViewport(data, viewer) {
    ...
    for (const [i, alloc] of data.segments[
      viewer.activeSegment
    ].allocations.entries()) {
      ...
      const size =
        alloc.size > 1024
          ? Math.round(alloc.size / 102.4) / 10 + "K"
          : alloc.size;
      ...
      box.style.right =
        ((viewport.cycles - alloc.alive_till) / viewport.cycles) * 100 + "%";
      box.style.backgroundColor = boxColors[i % boxColors.length];
      box.addEventListener("mouseover", () => {
        statusLineContainer.innerHTML = `<b>Origin:</b> ${
          alloc.origin.length > 32
            ? alloc.origin.substring(0, 32) + "…"
            : alloc.origin
        }
          | <b>Size:</b> ${alloc.size}
      ...
      box.addEventListener("click", (e) => {
        ...
        document.querySelector(".mondrian-sidepanel-origin").value =
          alloc.origin;
        document.querySelector(".mondrian-sidepanel-size").value = alloc.size;
        document.querySelector(".mondrian-sidepanel-offset").value =
          alloc.offset;
        document.querySelector(".mondrian-sidepanel-allocated").value =
          alloc.alive_from;
        document.querySelector(".mondrian-sidepanel-freed").value =
          alloc.alive_till;
    ...
    if (viewer.viewportMinCycle > 0) {
      ...
      shadow.style.left = "0%";
      shadow.style.width =
        (viewer.viewportMinCycle / viewport.cycles) * 100 + "%";
    ...
    if (viewer.viewportMaxCycle < viewport.cycles) {
      ...
      shadow.style.width =
        ((viewport.cycles - viewer.viewportMaxCycle) / viewport.cycles) * 100 +
        "%";
      viewerContainer.appendChild(shadow);
    }
  }
