...
export class ConfigObj {
  ...
  public updateBaseModelField(oldname: string, newname: string): Thenable<void> {
    ...
    const section: string = getSection(oldname);
    const kSection: CfgKeys = section as keyof Cfg;

    if (this.rawObj[kSection].input_path &&
        this.getFullPath(this.rawObj[kSection].input_path) === oldname) {
      this.rawObj[kSection].input_path = newname;
    } else {
      Logger.warn('ConfigObject', `Cannot update base model field: ${oldname} not found`);
    }
