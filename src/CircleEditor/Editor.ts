...
export class AttributeEditor implements Editor {
  ...
  public edit(inputBuiltinCode: string, attr: any): void {
    if (inputBuiltinCode === undefined || this._subgraphIdx === -1 || this._operatorIdx === -1) {
      Balloon.error('input data is undefined', false);
      return;
    }
    const operator: any = this._model.subgraphs[this._subgraphIdx] ?.operators[this._operatorIdx];
    ...
    const builtinOpEnumKeys = Object.keys(Circle.BuiltinOperator).filter((k) => isNaN(Number(k)));
    ...
    if (builtinCode === COSTUM_OP_CODE) {
    ...
    } else { /* builtinCode !== COSTUM_OP_CODE */
      let inputBuiltinCodeNormalized: any = inputBuiltinCode.toUpperCase() + 'OPTIONS';
      ...
      operator.builtinOptionsType = Types.BuiltinOptionsType[inputBuiltinCodeNormalized];
      ...
      if (operator.builtinOptionsType === Circle.BuiltinOptions.NONE) {
        ...
        return;
      } else { /* operator.builtinOptionsType !== Circle.BuiltinOptions.NONE */
      ...
      if (circleTypeKeys.find(element => element === attrType) === undefined) {
        switch (attrType) {
          case 'int32[]':
          case 'int16[]':
          case 'uint32[]':
          ...
          default: {
            ...
            operator.builtinOptions[targetKey] = Types.NormalType[attrType](attrValue);
          }
        }
      } else {  // attrType is CircleType
        operator.builtinOptions[targetKey] = Types.CircleType[attrType][attrValue];
      }
    }
  }
...
export class TensorEditor implements Editor {
  ...
  public edit(tensorName: string, args: any): void {
    ...
    const targetTensor = this._model ?.subgraphs[this._subgraphIdx] ?.tensors[tensorIdx];
