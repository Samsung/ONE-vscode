...
class DummyExecutor extends ExecutorBase {
  public getExecutableExt(): string[] {
    return ["dummy"];
  }

  public runInference(_model: string, options: string[]): Command {
    console.log("Dummy Inference")

    let dummyCommand = new Command("dummy-infer", ["--loadable", _model, ...options]);

    console.log(`Command: ${dummyCommand.str()}`)
    return dummyCommand;
  }
};
