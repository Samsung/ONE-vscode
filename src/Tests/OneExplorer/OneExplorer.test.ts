...
import {
  _unit_test_BaseModelNode as BaseModelNode,
  _unit_test_ConfigNode as ConfigNode,
  _unit_test_NodeFactory as NodeFactory,
  _unit_test_NodeType as NodeType,
  _unit_test_OneNode as OneNode,
  _unit_test_ProductNode as ProductNode,
} from "../../OneExplorer/OneExplorer";
  ...
  suite("OneExplorer", function () {
    ...
    suite("#NodeFactory", function () {
      test("NEG: create a directory node with attributes", function () {
        assert.throw(() => {
          NodeFactory.create(NodeType.directory, "", undefined, {
            ext: ".directory",
          });
      ...
      test("NEG: create a base model node outside workspace", function () {
        ...
        // Validation
        {
          ...
          assert.strictEqual(
            baseModelNode.openViewType,
            BaseModelNode.defaultOpenViewType
          );
          assert.strictEqual(baseModelNode.icon, BaseModelNode.defaultIcon);
          assert.strictEqual(
            baseModelNode.canHide,
      ...
      test("create a config node", function () {
        ...
        // Validation
        {
          ...
          assert.strictEqual(
            ...
            ConfigNode.defaultOpenViewType
          );
      ...
      test("create a node with parent", function () {
        ...
        const directoryNode = NodeFactory.create(
          NodeType.directory,
          directoryPath,
          undefined
        );
