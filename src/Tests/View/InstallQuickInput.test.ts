...
suite("View", function () {
  suite("InnerButton", function () {
    suite("#constructor()", function () {
      test("is constructed with InnerButton", function () {
        const innerButton = new InnerButton(
  ...
  suite("InstallQuickInput", function () {
    ...
    suite("#getToolchainEnv()", function () {
      ...
      test("NEG: throw error when ToolchainEnv is undefined, but toolchainType defined", function () {
        let quickInput = new InstallQuickInput();
    ...
    suite("#getToolchainType()", function () {
      ...
      test("NEG: throw error when ToolchainType is undefined, but toolchainEnv defined", function () {
        let quickInput = new InstallQuickInput();
        quickInput.toolchainEnv = toolchainEnv;
        assert.throw(() => {
          quickInput.getToolchainType();
    ...
    suite("#getToolchainEnvFromGlobal()", function () {
      test("gets toolchain env from Global", function () {
        let quickInput = new InstallQuickInput();
        assert.strictEqual(
          quickInput.getToolchainEnvFromGlobal(backendName),
          toolchainEnv
        );
      });
      test("NEG: gets invalid toolchain env from Global", function () {
        let quickInput = new InstallQuickInput();
        assert.strictEqual(
          quickInput.getToolchainEnvFromGlobal("invalid"),
          undefined
        );
      });
    });
    ...
    suite("#getToolchainTypes()", function () {
      test("gets toolchain types", function () {
        ...
        quickInput.toolchainEnv = toolchainEnv;
        assert.strictEqual(
          quickInput.getToolchainTypes().length,
          toolchainEnv.getToolchainTypes().length
        );
      });
      ...
      test("NEG: throw error by getToolchainEnv when ToolchainEnv is undefined, but toolchainType defined", function () {
        let quickInput = new InstallQuickInput();
        quickInput.toolchainType = toolchainType;
        assert.throw(() => {
          quickInput.getToolchainTypes();
      ...
      test("NEG: throw error by getToolchainEnv when ToolchainEnv is undefined, but toolchain defined", function () {
        let quickInput = new InstallQuickInput();
        quickInput.toolchain = toolchain;
        assert.throw(() => {
          quickInput.getToolchainTypes();
      ...
      test("NEG: throw error by getToolchainEnv when ToolchainEnv is undefined, but version defined", function () {
        let quickInput = new InstallQuickInput();
        quickInput.version = version;
        assert.throw(() => {
          quickInput.getToolchainTypes();
      ...
      test("NEG: throw error by getToolchainEnv when ToolchainEnv is undefined, but error defined", function () {
        let quickInput = new InstallQuickInput();
        quickInput.error = "some error";
        assert.throw(() => {
          quickInput.getToolchainTypes();
      ...
      test("NEG: throw error by getToolchainEnv when ToolchainEnv is undefined, but all others defined", function () {
        let quickInput = new InstallQuickInput();
        quickInput.toolchainType = toolchainType;
        quickInput.toolchain = toolchain;
        quickInput.version = version;
    ...
    suite("#changeCurrentStepBefore()", function () {
      test("changes from pickBackend step to previous step", function () {
        ...
        const state = {
          ...
          current: InstallQuickInputStep.unset,
        } as InstallQuickInputState;
        const enumName =
          InstallQuickInputStep[InstallQuickInputStep.pickBackend];
      ...
      test("changes from pickVersion step to previous step", function () {
        ...
        const enumName =
          InstallQuickInputStep[InstallQuickInputStep.pickVersion];
        quickInput.changeCurrentStepBefore(enumName, state);
        assert.strictEqual(state.current, InstallQuickInputStep.pickType);
      });
      test("NEG: changes to previous step with invalid step name using changeCurrentStepBefore", function () {
        const quickInput = new InstallQuickInput();
        const names = ["item0"];
        const items = quickInput.getQuickPickItems(names);
        assert.strictEqual(items.length, 1);
      ...
      test("NEG: changes to previous step with almost legal step name using changeCurrentStepBefore", function () {
        const quickInput = new InstallQuickInput();
        const names = ["item0"];
        const items = quickInput.getQuickPickItems(names);
        assert.strictEqual(items.length, 1);
        ...
        const invalidStepName =
          InstallQuickInputStep[InstallQuickInputStep.pickVersion] + "A";
        expect(function () {
          quickInput.changeCurrentStepBefore(invalidStepName, state);
        }).to.throw(`wrong stepName: ${invalidStepName}`);
      });
    ...
    suite("#changeCurrentStepAfter()", function () {
      ...
      test("NEG: changes to previous step with invalid step name using changeCurrentStepAfter", function () {
        const quickInput = new InstallQuickInput();
        const names = ["item0"];
        const items = quickInput.getQuickPickItems(names);
        assert.strictEqual(items.length, 1);
      ...
      test("NEG: changes to previous step with almost legal step name using changeCurrentStepAfter", function () {
        const quickInput = new InstallQuickInput();
        const names = ["item0"];
        const items = quickInput.getQuickPickItems(names);
        assert.strictEqual(items.length, 1);
    ...
    suite("#getMultiSteps()", function () {
      ...
      test("gets MultiSteps in pickBackend state", function () {
        ...
        const state = {
          selectedItem: undefined,
          current: InstallQuickInputStep.pickBackend,
        } as Partial<InstallQuickInputState>;
        const inputSteps = quickInput.getMultiSteps(state);
      ...
      test("gets MultiSteps in pickType state", function () {
        ...
        const state = {
          selectedItem: undefined,
          current: InstallQuickInputStep.pickType,
        } as Partial<InstallQuickInputState>;
        const inputSteps = quickInput.getMultiSteps(state);
